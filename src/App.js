...
class App extends Component {
  ...
  handleThumbnailClick = thumbnailCategory => {
    if (thumbnailCategory === this.state.imageToMatch) {
      this.setState(prevState => ({
  ...
  renderThumbnails = () => {
    ...
    const activeTabImages = imagesList.filter(
      image => image.category === activeTabId
    )
    return activeTabImages.map(image => (
      <li key={image.id}>
        <img
          src={image.thumbnailUrl}
          alt="thumbnail"
          onClick={() => this.handleThumbnailClick(image.category)}
        />
      </li>
    ))
  }
    ...
    return tabsList.map(tab => (
      <li key={tab.tabId}>
        <button
          ...
          className={activeTabId === tab.tabId ? 'active' : ''}
        >
          {tab.displayText}
        </button>
      </li>
  ...
  render() {
    const { score, time, imageToMatch, isGameOver } = this.state
    ...
    return (
      <div className="app-container">
        <div className="match-game-container">
          ...
          <ul className="tabs-container">{this.renderTabs()}</ul>
          <ul className="thumbnails-container">{this.renderThumbnails()}</ul>
          ...
          {isGameOver ? (
            ...
            <>
              <p className="time-remaining">Time Remaining: {time}</p>
              {time === 0 ? (
                <>
                  <img
                ...
                <img className="image-to-match" src={imageToMatch} alt="match" />
              )}
            </>
          )}
        </div>
